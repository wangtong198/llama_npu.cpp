...
// 已经决定要用的那个 token（模型输出或你喂进去的 prompt token） 定下来之后，
// 把它 告知整条采样管线：推理预算、（在允许时）GBNF 语法状态、
// 以及主采样链里的各种策略（重复惩罚、温度、top‑p、dist 的 RNG 等），
// 并在 common_sampler 里记下 最近 token 历史（prev），供下一步采样用。
void common_sampler_accept(struct common_sampler * gsmpl, llama_token token, bool accept_grammar) {
    if (!gsmpl) {
        return;
    }

    // 析构时在作用域结束时把这段调用的耗时记进统计
    ...
    // 总是调用（rbudget 可能为空，库内会 no‑op）：推进 reasoning budget（若启用）的内部状态。
...
// 从 common_sampler 里维护的 prev（最近已 accept 的 token 历史） 取出 末尾最多 n 个 token，
// 按词表 common_token_to_piece 转成 UTF-8 片段并 拼成一串字符串 返回。
