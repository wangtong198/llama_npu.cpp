...
common_init_result::common_init_result(common_params & params) :
    ...
    if (params.fit_params) {
        ...
        // 可能降低 cparams.n_ctx（条件：n_ctx==0、训练 ctx 大于下限、且全局亏空；并对齐 256）
        // 单卡可能仅靠降 ctx 就 return
        // 否则自动求 n_gpu_layers、多卡 tensor_split、tensor_buft_overrides（整层/部分层 FFN/MoE 正则 + CPU 或下一卡 buft）
...
bool common_prompt_batch_decode(
    ...
    const std::vector<llama_token> & tokens,        // 本轮要解码的 token 序列
                               int & n_past,        // 解码前的“已占用序列长度”，函数结束时累加上本轮实际解码的个数
                               int   n_batch,       // 配置的最大 batch；仅在某些保存 state 的路径下用于断言整段 tokens 能放进 batch
