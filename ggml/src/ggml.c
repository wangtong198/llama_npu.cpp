...
static struct ggml_tensor * ggml_new_tensor_impl(
    ...
    // 这里只展开一层链；更深层一般由构图方式保证或多次创建时处理
    if (view_src != NULL && view_src->view_src != NULL) {
        view_offs += view_src->view_offs;
        view_src   = view_src->view_src;
    }

    // data_size: ggml_tensor 的逻辑元素在内存里按类型规则展开后需要的 总字节数
...
static struct ggml_tensor * ggml_rope_impl(
    ...
    // op_params[0]：n_past（历史字段，当前实现未使用）,兼容旧接口/序列化路径保留。
    // op_params[1]：n_dims, 参与 RoPE 旋转的维度数。 必须 <= ne0 且为偶数。
    // op_params[2]：mode, RoPE 类型/模式位标志。例如 NORMAL / NEOX / MROPE / VISION / IMROPE。
    // op_params[3]：n_ctx（历史字段，当前实现未使用）,兼容旧接口/序列化路径保留。
    // op_params[4]：n_ctx_orig, 原始上下文长度。YARN 等上下文扩展计算要用
    // op_params[5]: freq_base（float）,频率基数（常见 10000 或更大如 1e6）。
    // op_params[6]：freq_scale（float）,频率缩放系数（影响位置相位增长速度）。
    // op_params[7]：ext_factor（float）,上下文外推/扩展相关系数（YARN 系参数之一）。
    // op_params[8]：attn_factor（float）,注意力相关的额外缩放因子（配合扩展策略）。
    // op_params[9]：beta_fast（float）, YYARN 修正区间的“快”边界参数。
    // op_params[10]：beta_slow（float）, YARN 修正区间的“慢”边界参数。
    // op_params[11]：sections, mrope 分段参数（sections 一共 4 个）
...
// - 哈希表用素数大小，通常能减少冲突，分布更均匀
// - 这些素数都接近 2 的幂，容量增长比较平滑
// - ggml 里像图结构的 hash set 会用它来决定内部数组大小
...
// 从根张量出发，沿 src 指向操作数（父结点）对表达式 DAG 做深度优先遍历。
//
// 本过程在构建 / 扩展 ggml_cgraph 时负责：
// 1) 用 visited_hash_set 对每个 ggml_tensor 去重，每指针只出现一次；返回值为其哈希表槽下标，
//    与 use_counts、grads 等并行数组下标一致。
// 2) 可选地为参与当前前向路径的结点打上 GGML_TENSOR_FLAG_COMPUTE （见 ggml_build_forward_select：
//    某些根最初以 compute==false 展开，后续再以 compute==true 进入时，需沿已登记结点向上传播该标志）。
// 3) 统计扇出：use_counts[slot] 表示有多少条「消费者边」引用该槽对应张量（供分配 / 原地更新与后端使用）。
// 4) 将每个首次见到的张量追加到 cgraph->nodes（可执行算子、参数、loss）或 cgraph->leafs（GGML_OP_NONE
//    且非 PARAM 的常量 / 输入），顺序由 DFS 与 cgraph->order 共同决定。
static size_t ggml_visit_parents_graph(struct ggml_cgraph * cgraph, struct ggml_tensor * node, bool compute) {
    // 仅非 GGML_OP_NONE 的结点才算调度意义上的「可计算子图」；叶子在下面分支单独处理。
    if (node->op != GGML_OP_NONE && compute) {
        node->flags |= GGML_TENSOR_FLAG_COMPUTE;
    }

    // 张量指针 -> visited_hash_set 中稳定槽位（亦为 use_counts 等数组下标）。为node 找一个 hash
    const size_t node_hash_pos = ggml_hash_find(&cgraph->visited_hash_set, node);
    GGML_ASSERT(node_hash_pos != GGML_HASHSET_FULL);

    // 看哈希表里 下标 node_hash_pos 这个槽是不是已经被占用，如果被占用，则说明这个张量已经访问过。
    if (ggml_bitset_get(cgraph->visited_hash_set.used, node_hash_pos)) {
        // 再次进入：本次图构建中已登记过该张量。不得重复插入 nodes/leafs，也不得重复累计边上的 use_counts。

        if (compute) {
            // 当前结点已确认落在需要计算的链路上；保证所有操作数都带上 COMPUTE，即便它们最初是在
            // compute==false 时被发现（多根图场景）。
    ...
    // 首次对该张量访问：先占用哈希槽并将引用计数置 0，再递归，以便父结点对子边做 use_counts[src]++ 时子槽已存在。
    cgraph->visited_hash_set.keys[node_hash_pos] = node;
    ggml_bitset_set(cgraph->visited_hash_set.used, node_hash_pos);
    cgraph->use_counts[node_hash_pos] = 0;

    for (int i = 0; i < GGML_MAX_SRC; ++i) {
        // 操作数遍历顺序影响 nodes[] / leafs[] 的 DFS 顺序，进而可能影响求值侧启发式。
        const int k =
        ...
        if (src) {
            ...
            // 本结点到 src 的一条消费者边；src 的扇出在整张图上累加。
            cgraph->use_counts[src_hash_pos]++;
        }
    }

    // op == GGML_OP_NONE：在图里没有挂任何可执行算子，
    //  也就是说它不是某次 ggml_mul_mat / ggml_add 等在 本 context 里建出来的「运算结果」，而是表达式的端点；
    // 且没有 GGML_TENSOR_FLAG_PARAM：不是按「可训练参数」那种语义登记的叶子（带 PARAM 的会走 else，进 nodes）
    if (node->op == GGML_OP_NONE && !(node->flags & GGML_TENSOR_FLAG_PARAM)) {
        // 图输入或常量：无算子可执行，且非可训练参数。
        ...
        // 可计算结点（含带 PARAM 的 GGML_OP_NONE，或实现前向/反向核的任意 op）。
...
static void ggml_build_forward_impl(struct ggml_cgraph * cgraph, struct ggml_tensor * tensor, bool expand, bool compute) {
    ...
    // 这次扩展至少往 nodes[] 里新塞了东西（通常是根及其子表达式里那些 op != NONE 或 NONE+PARAM 的张量）
    // 在这次 DFS 里，最后被加入 nodes[] 的必须是这次 ggml_build_forward_expand 传入的「起点」tensor
    if (n_new > 0) {
...
// use_counts = cgraph0->use_counts: 即便切出一个 nodes 子区间，这些节点所依赖的 tensor 的使用计数，仍然必须来自原图同一份统计
// visited_hash_set = cgraph0->visited_hash_set: 
//      切 split 的时候，节点还是原来的那些 tensor，不是复制出来的新 tensor，所以它们必须继续使用原图那套 hash 映射
