...
struct buffer_address { // 张量在虚拟连续空间中的相对地址（第几个 chunk + chunk 内偏移）
...
// 每个 chunk 的空闲块表和峰值使用量
struct tallocr_chunk {
    struct free_block free_blocks[MAX_FREE_BLOCKS];     // 空闲区间描述
    int n_free_blocks;
    size_t max_size;    // 用于决定每个 chunk 应申请多大物理 buffer。
};

// 这是“先规划地址，不立刻绑实际 tensor->data”的层。
// reserve 阶段通过 best-fit + 合并空闲块得到较优重用；
struct ggml_dyn_tallocr {
    size_t alignment;
    size_t max_chunk_size;
    struct tallocr_chunk * chunks[GGML_VBUFFER_MAX_CHUNKS];     // 每个 chunk 的空闲块表和峰值使用量
    int n_chunks;
...
struct vbuffer {
    // 一个 vbuffer 代表“逻辑连续地址空间”，底层可由多个 ggml_backend_buffer_t chunk 组成。
    // 配合 buffer_address {chunk, offset} 表示张量位置。
    // 这么做是为了支持大图分配时按 chunk 增长，避免单块过大失败。
...
struct hash_node {
    int n_children;     // 还有多少下游消费者，归零后可释放；
    int n_views;        // 还有多少 view 引用
    int buffer_id;      // 分配到哪个 bufts/buffers 槽位
    struct buffer_address addr;     // 在该槽位的 {chunk, offset}
    bool allocated;     // 是否由 galloc 自己管理并已分配
};

// 单 tensor 的“绑定快照”（在哪个 buffer_id，地址在哪里，历史最大尺寸）
struct tensor_alloc {
    int buffer_id;
    struct buffer_address addr;
...
// 仅包装 leaf 的 tensor_alloc
struct leaf_alloc {
    struct tensor_alloc leaf;
};

// 同时记录 node 的 dst 和每个 src 的快照，用于重放绑定和校验是否需要 re-reserve。
struct node_alloc {
    struct tensor_alloc dst;
    struct tensor_alloc src[GGML_MAX_SRC];
};

// 给一张 ggml_cgraph 的 leaf/node 规划并复用内存地址
// 支持多 backend buffer type
// 把“规划结果”缓存下来，后续快速重新绑定 tensor 指针

// 设计的原理（重点）
//     1 规划与物理分配分离
//       ggml_dyn_tallocr 负责地址规划；
//       vbuffer 负责实际 backend buffer。
//       好处：先算“需要多大/怎么排”，再决定是否重分配，减少频繁 malloc/设备分配。

//     2 多 backend 统一抽象为 buffer_id 槽位
//       bufts/buffers/buf_tallocs 都按同一 buffer_id 对齐。
//       上层只管 tensor 属于哪个 buffer_id，不用每次分支判断 backend 实现细节。

//     3 哈希索引保证 O(1) 级 tensor 元数据访问
//       通过 tensor* -> slot 快速拿到 hash_node，便于在遍历图时做 children/views 计数和生命周期管理。

//     4 node_allocs/leaf_allocs 快照化，服务“快速重绑定”
//       一次 reserve 后，后续相似图可直接 alloc_graph 重绑地址，只有图变化或尺寸变化时才需要再 reserve。

//     5 chunk 化是为设备分配现实约束服务
//       单块太大可能失败、不同 backend 上限不同；chunk + max_chunk_size + 最后 chunk 放宽策略，提升成功率和弹性。

//     6 同 buft 去重复用 allocator/物理 buffer
//       多个逻辑槽位若底层是同一种 buffer type，可共享 buf_tallocs（甚至复用 vbuffer），避免重复维护两套池。
struct ggml_gallocr {
    // 每个逻辑 buffer 槽位对应的 ggml_backend_buffer_type_t
    // backend 的“内存类型描述符”（比如 CPU host、某设备 memory type）
    ggml_backend_buffer_type_t * bufts; // [n_buffers]
    // 每个槽位实际分配出来的 vbuffer*（物理 buffer 集合）
    // reserve 后可能新建/扩容；alloc_graph 时会在这些 buffer 上重置并绑定 tensor。
    struct vbuffer ** buffers; // [n_buffers]       A. 物理 buffer 层：struct vbuffer
    // 每个槽位对应一个动态子分配器 ggml_dyn_tallocr*，负责“地址规划”（offset/chunk）
    // 注意：如果多个槽位 buft 相同，会复用同一个 ggml_dyn_tallocr（构造函数里有去重逻辑）。
    struct ggml_dyn_tallocr ** buf_tallocs; // [n_buffers]      B. 地址规划层：struct ggml_dyn_tallocr
    // 槽位数量；通常和 scheduler 传入 backend/buft 数一致。
    int n_buffers;

    // 用 tensor 指针做 key 的哈希索引层
    // hash_set 管“槽位占用和 key”；
    // hash_values[i] 存该 tensor 的分配元数据
    struct ggml_hash_set hash_set;
    struct hash_node * hash_values; // [hash_set.size]

    // 按“当前已规划图”的 node/leaf 数量缓存分配快照
    // 后续 ggml_gallocr_alloc_graph() 可直接按快照把 tensor 绑定到地址，避免每次重新全量规划。
...
// 在图分配器里，为某个 node 张量在指定的 buffer_id 对应的内存池里落实/记录一块存放地址
//   （规划在 ggml_dyn_tallocr 里，必要时做 in-place 复用）
...
static void ggml_gallocr_alloc_graph_impl(ggml_gallocr_t galloc, struct ggml_cgraph * graph, const int * node_buffer_ids, const int * leaf_buffer_ids) {
    ...
    for (int i = 0; i < graph->n_leafs; i++) {
        ...
        // 把 leaf 这一条线 先放进哈希表里的规划
        // （多数权重因已有 buffer，这里常常是快速 return；需要 galloc 管的 leaf 会真正划块）
    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            ...
            if (src->flags & GGML_TENSOR_FLAG_INPUT) {
                // 保证 显式输入张量在真正按拓扑模拟释放/复用前，已经被规划好
    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            ...
            // 对每个 node 先 allocate 所有 parent
            ggml_gallocr_allocate_node(galloc, parent, buffer_id);
        }

        // allocate node
        // 为 本算子的输出在对应 pool 里拿地址
...
// 为当前这一段张量区间申请「一整块」给定 buft 的 backend buffer，
// 再在这块内存里按顺序给区间内的张量分配偏移或初始化视图，
// 并把新 buffer 追加到调用方维护的 buffers 数组里。
static bool alloc_tensor_range(struct ggml_context * ctx,
        struct ggml_tensor * first, struct ggml_tensor * last,
        ggml_backend_buffer_type_t buft, size_t size,
        ggml_backend_buffer_t ** buffers, size_t * n_buffers) {

    // 该 buft 为这一段张量区间申请一块大小为 size 的 buffer
    ...
    // 把这块 buffer 记入 *buffers
    *buffers = realloc(*buffers, sizeof(ggml_backend_buffer_t) * (*n_buffers + 1));
    (*buffers)[(*n_buffers)++] = buffer;

    // 在 buffer 上建一个 子分配器（tensor allocator），后面从同一块物理内存里按张量切 data 指针。
...
static ggml_backend_buffer_t ggml_backend_alloc_ctx_tensors_from_buft_impl(
    ...
    GGML_ASSERT(ggml_get_no_alloc(ctx) == true);    // 在为 buft 设置 context 时，no_alloc 必须为 true
    ...
    *nbytes_total = 0;
    ...
    for (struct ggml_tensor * t = first; t != NULL; t = ggml_get_next_tensor(ctx, t)) {
        ...
        // 后端对「单次 alloc_buffer 能申请的最大字节数」往往有限制（ggml_backend_buft_get_max_size，可能是驱动/API 限制，或实现上故意封顶）。
        // 若把所有张量的大小加总后 一次 alloc_buffer，可能失败或不符合后端约定。

        // 因此这里做的是：在不超过 max_size 的前提下分段申请多块 buffer；
        // 若最终有多块，后面会用 ggml_backend_multi_buffer_alloc_buffer 包成一个对外仍像一块的逻辑 buffer
        if (cur_buf_size > 0 && (cur_buf_size + this_size) > max_size) {
            // allocate tensors in the current buffer
            // 加上这个tensor的大小超出了max_size，所以这里先为这个 tensor 之前的 tensor
            // buffers 增加了一个 ggml_backend_buffer_t 对象，同时存储空间的个数增加了 1
            if (!no_alloc && !alloc_tensor_range(ctx, first, t, buft, cur_buf_size, &buffers, &n_buffers)) {
                return NULL;
            }
            first = t;  // 下次从这个 tensor 开始分配
    ...
    if (n_buffers == 1) {
        ...
        // 使多buffers场景在使用时，和n_buffer == 1有相同的使用方法
...
ggml_backend_buffer_t ggml_backend_alloc_ctx_tensors_from_buft(struct ggml_context * ctx, ggml_backend_buffer_type_t buft) {
    ...
    if (ggml_backend_buft_is_meta(buft)) {
        // 对于一个张量，如果需要拆分到多个 device 上，那么这个张量的 buffer type 就是 meta buffer type
        // 比如一个张量需要拆分到CPU和GPU上，那么这个张量的 buffer type 就是 meta buffer type
