...
// 识别是否为 Meta 复合设备的 buffer type：由多个子后端的 buft 组合而成，
// 用于在多设备上做张量并行时在各自子 buffer 上分配数据（典型如多块 GPU；亦可是 CPU+GPU 等组合）。
// 注意：仅表示 buft 的实现族系为 meta；并不是说「凡跨设备切分的张量」都必然是这一种 buft。
