...
// 在 GGML 的 ggml_hash_set 里为给定 ggml_tensor * key 找一个槽位下标 i`。
static size_t ggml_hash_find(const struct ggml_hash_set * hash_set, const struct ggml_tensor * key) {
...
struct ggml_cgraph {
    // 这是这份图容器的容量上限，不是当前已使用数量。
    int size;    // maximum number of nodes/leafs/grads/grad_accs
    // 当前图里已经收集了多少个可计算节点: 
    // 1. 有 op 的张量; 
    // 2. 带 GGML_TENSOR_FLAG_PARAM 的特殊张量: 
    int n_nodes; // number of nodes currently in use
    // 当前图里已经收集了多少个叶子结点。
    // 1. 输入张量; 
    // 2. 常量张量; 
    // 3. op == GGML_OP_NONE 且不是 PARAM 的张量
    int n_leafs; // number of leafs currently in use

    // 这是一个张量指针数组，保存图里的可执行结点
    ...
    // 这是一个使用计数数组，记录每个张量被用作输入多少次。
    // 主要给图执行和内存回收用。
    // 比如一个张量被后续多个节点引用：
    // - 它的 use_counts 就会比较大
    // - 执行时可以据此判断这个张量什么时候还能安全释放/复用
    int32_t             * use_counts;// number of uses of each tensor, indexed by hash table slot

    // 这是构图时用的去重/访问记录表
    // 因为 DFS 遍历图时，一个张量可能从多个路径被访问到。
