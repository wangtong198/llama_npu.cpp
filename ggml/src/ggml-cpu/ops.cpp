...
static void ggml_compute_forward_get_rows_f32(
    ...
    const int64_t nc = ne00;    // 词表中每一“行”里连续 float 的个数（嵌入维度）。
    const int64_t nr = ggml_nelements(src1);    // 要取多少行

    assert(ne0  == nc);
    // 一个batch组时，ne02 == ne11 == 1

    // 多组/多序列并行
    // src0：ne = [4096, 151936, 4, 1]
    // src1：ne = [128, 4, 1, 1]
    // 这里 ne02 == ne11 == 4
    assert(ne02 == ne11);
...
static void ggml_compute_forward_soft_max_f32(
    ...
    // 为 ALiBi（Attention with Linear Biases） 
    // 准备 每个 head 不同的斜率 slope，用来乘到 mask 偏置上
    // （见后面 wp[i] += slope * mp_*[i]）。
...
static void ggml_compute_forward_rope_flt(
    ...
    const ggml_tensor * src0 = dst->src[0];     // Q / K
    const ggml_tensor * src1 = dst->src[1];     // position IDs
    const ggml_tensor * src2 = dst->src[2];     // frequency factors，可选
    ...
    //const int n_past     = ((int32_t *) dst->op_params)[0];
    const int n_dims     = ((int32_t *) dst->op_params)[1]; // n_dims, 参与 RoPE 旋转的维度数。 必须 <= ne0 且为偶数。
    const int mode       = ((int32_t *) dst->op_params)[2]; // mode, RoPE 类型/模式位标志。例如 NORMAL / NEOX / MROPE / VISION / IMROPE。
    //const int n_ctx      = ((int32_t *) dst->op_params)[3];
    const int n_ctx_orig = ((int32_t *) dst->op_params)[4]; // 原始上下文长度。YARN 等上下文扩展计算要用

    memcpy(&freq_base,   (int32_t *) dst->op_params +  5, sizeof(float));   // 频率基数（常见 10000 或更大如 1e6）。 也就是rope_theta配置项
    memcpy(&freq_scale,  (int32_t *) dst->op_params +  6, sizeof(float));   // 频率缩放系数（影响位置相位增长速度）
    memcpy(&ext_factor,  (int32_t *) dst->op_params +  7, sizeof(float));   // 上下文外推/扩展相关系数（YARN 系参数之一）
    memcpy(&attn_factor, (int32_t *) dst->op_params +  8, sizeof(float));   // 注意力相关的额外缩放因子（配合扩展策略）。
    memcpy(&beta_fast,   (int32_t *) dst->op_params +  9, sizeof(float));   // YARN 修正区间的“快”边界参数。
    memcpy(&beta_slow,   (int32_t *) dst->op_params + 10, sizeof(float));   // YARN 修正区间的“慢”边界参数。
    memcpy(&sections,    (int32_t *) dst->op_params + 11, sizeof(int)*4);   // mrope 分段参数（sections 一共 4 个）
