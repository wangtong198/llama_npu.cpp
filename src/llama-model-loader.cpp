...
llama_model_loader::llama_model_loader(
        ...
        bool use_direct_io,
        bool check_tensors,
        bool no_alloc,      // 这里使用的common_params结构体默认的值：false
    ...
    if (!fname.empty()) {
        ...
        // 返回了一个 gguf_context实例，保存了 gguf 模型中的基础信息、kv信息，tensor信息（不包括tensor数据）
        // 副作用： 会给params.ctx 指向的指针一个 ggml_context 实例，设置了其中的 mem_size 和 mem_buffer 空间，但并没有为每个tensor分配数据空间
        // 由于 params.ctx 引用了上面的  ctx的地址，所以ctx也会被设置为 ggml_context 实例，其中的 no_alloc：true
        metadata_ptr.reset(gguf_init_from_file(fname.c_str(), params));     // 这里传入的 no_alloc 为true
        ...
        // 对每个非空的 fname（以及分片的每个 shard 文件名），会先 gguf_init_from_file(..., gguf_init_params{ no_alloc:true, ctx:&ctx })，
        // 随后在 contexts 里 emplace_back 的正是 这一次为「该 GGUF」建出来的那一个 ggml_context *
        ...
        for (ggml_tensor * cur = ggml_get_first_tensor(ctx); cur; cur = ggml_get_next_tensor(ctx, cur)) {
            ...
            // weights_map: 按名字索引「每个权重属于哪个 GGUF shard、在该文件内的数据偏移」，并挂上一份张量描述
...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        // 这里的 ctx_map 是在CPU进程中维护的，
        // ctx_map[buft]：表示所有 计划放在这种 buffer 类型上 的权重，它们的 ggml 张量描述符 都登记在这个 ggml_context 里。
        // 其中 ctx 的 mem_buffer 只是 主机上的元数据池；设备上的大块权重 是后面 ggml_backend_buffer 的事
        auto it = ctx_map.find(buft);
        if (it == ctx_map.end()) {
            // 每种实际会用到的 buffer 类型各有一个 context，
            // 用来在该 context 里 ggml_dup_tensor / ggml_new_tensor_* 建出「图里的张量节点」（元数据、shape、stride 等）
            // one ggml context per buffer type
            int max_n_tensors = n_tensors;      // n_tensors：GGUF 里权重张量总数
            max_n_tensors += 1;                 // 输出层与 embedding 绑权重时多出来的 duplicate output 张量。
            max_n_tensors += hparams.n_layer*2; // 每层可能重复的 rope 频率相关张量
            if (files.empty()) {
            ...
            ggml_init_params params = {
                ...
                // 这个 context 不负责给张量分配权重数据内存；
                // 数据由后面的 ggml_backend_buffer（按 buft）去分配。
                // 这里只要一块足够大的 元数据区（tensor overhead arena）。
                /*.no_alloc   =*/ true,
            };
    ...
    auto buft_for_tensor = [&](ggml_tensor * t_meta) -> ggml_backend_buffer_type_t {
        ...
        // 不少 LM 里 token_embd 和 output（lm_head） 共用同一份权重矩阵：
        // GGUF 里往往只有一个张量（例如 token_embd.weight），没有单独的 output.weight。
        ...
        // check overrides
        // fit 或者手动 offload时，修改buffer type
        ...
        if (!buft) {
            // 根据 t_meta 所属于的 OP， 按照先后顺序遍历buft_list，观察在哪种 device 上是被支持的，就选中最先支持 t_meta 的那一个 buft
        ...
        // mmap 时不要使用「加速器设备名下的那种 host_buffer_type」，改成 CPU 后端默认 buft。
        ...
        // 统计并记录「没有用上该层首选 buffer type 的权重张量」
        // 留给 done_getting_tensors() 里打 DEBUG 日志用。
    ...
    // 为当前tensor所在的buffer type分配一个 ggml_context，
    // 这里分配的 ggml_context 是 CPU 进程里的，不是设备上的
    // 为了将权重张量对象元数据登记到这个 ggml_context 里，后续通过 ggml_dup_tensor 复制 当前 tensor 的元信息
    ggml_context * ctx = ctx_for_buft(buft);
    ...
    // 在 buft 的 ctx 的 mem_buffer 中，根据 cur 这个tensor的size，dtype等信息，划分一块空间，为张量对象/元数据 占坑，
    // 复制 cur 的 type、ne、nb、名字 等元信息，使 计算图里的权重节点 与 GGUF 里描述 一致。
    // 不是为整块权重数据在 mem_buffer 里分配位置
...
// 在 use_mmap 为真 时「把每个 GGUF 文件 mmap 到进程地址空间」
// - 给每个文件建 llama_mmap
// - 记账 
// - mapping 容器 
// - 遍历 weights_map，把每个权重张量的 ggml_nbytes 累加到 size_data，给 加载进度条 / 百分比 用。
...
bool llama_model_loader::load_all_data(
    ...
    std::vector<no_init<uint8_t>> read_buf;     // 同步读盘时的临时缓冲（后面非 async 路径可能用到）。
    std::vector<std::future<std::pair<ggml_tensor *, bool>>> validation_result; // check_tensors 时异步校验结果的 future 列表。

    // staging 指的是：夹在「磁盘文件」和「GPU 显存」之间的中间一块（这里是多块轮换的）主机内存，
    // 用来 暂存 刚从文件读出来的数据，再交给 tensor_set_async 异步拷到 GPU。
    constexpr size_t n_buffers = 4;     // 异步上传用的 staging 块数量
    ...
    // upload_backend 是上面llambda执行后的返回值，由于设备不支持异步和event，所以会返回nullptr
    ...
    ggml_backend_t upload_backend = [&](const char * func) -> ggml_backend_t {
        ...
        return backend;
    }(__func__);    // 立刻调用这个 lambda， __func__ 是 C++ 标准里的预定义名字，展开为 当前所在函数 的名字字符串：load_all_data
    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        if (use_mmap) {
            ...
            if (ggml_backend_buffer_is_host(cur->buffer)) {     // 加载属于Host上的tensor的权重数据
    ...
    // 上面已经将 权重 写入到了具体的backend buffer中，这里需要释放掉这些临时申请的host buffer
