...
// 当前平台是否支持“把一段内存锁在物理 RAM 里、避免被换页出去”
// 在支持该可选特性的 Linux 等系统上会出现，用来走 POSIX 的 mlock / munlock 路径。
// 或者
// 当前是在 Windows 上编译，后面一般用 VirtualLock / VirtualUnlock 等 Win32 API，而不是 POSIX 的 mlock。
