...
llm_graph_context::llm_graph_context(const llm_graph_params & params) :
    ...
    n_layer          (hparams.n_layer_forward()),
    n_rot            (hparams.n_rot()),
    n_ctx            (cparams.n_ctx),
...
ggml_tensor * llm_graph_context::build_ffn(
   ...
   llm_ffn_gate_type   type_gate,
    ...
    if (gate) {
        switch (type_gate) {
            case LLM_FFN_SEQ:   // 串行，用于比较老的模型
                {
                    cur = build_lora_mm(gate, tmp);
                    cb(cur, "ffn_gate", il);
                } break;
            case LLM_FFN_PAR:   // 并行，用于比较新的模型
                {
...
ggml_tensor * llm_graph_context::build_moe_ffn(
         ggml_tensor * cur,                         // 进入本层 MoE FFN 的隐状态，形状 [n_embd, n_tokens]
         ggml_tensor * gate_inp,                    // 路由线性层权重,与 cur 相乘得到每个专家上的 logits：[n_expert, n_tokens]
         ggml_tensor * gate_inp_b,
         ggml_tensor * up_exps,     // 各专家的 up_proj 权重堆叠（mul_mat_id 用）。与当前隐状态相乘得到 [n_ff, n_expert_used, n_tokens]
         ggml_tensor * up_exps_b,
         ggml_tensor * gate_exps,   // 各专家的 gate_proj 权重（SwiGLU/GeGLU 等需要 gate 时）。可为 nullptr：无 gate 时
         ggml_tensor * gate_exps_b,
         ggml_tensor * down_exps,   // 各专家的 down_proj 权重。在激活之后把中间维投回 [n_embd, n_expert_used, n_tokens]
         ggml_tensor * down_exps_b,
         ggml_tensor * exp_probs_b, // 专家选择用偏置（DeepSeek V3 一类）
             int64_t   n_expert,    // 路由专家总数
             int64_t   n_expert_used,   // 每个 token 实际激活的专家个数（top-k 的 k）
     llm_ffn_op_type   type_op,     // 专家内的非线性/门控类型
                bool   norm_w,      // 是否对选中的 路由权重 按行归一（除以 top-k 权重和），避免和为 0 时还会 clamp。
               float   w_scale,     // 对归一后（若启用）权重的 额外标量缩放；为 0 或 1 时跳过缩放。
        llama_expert_gating_func_type gating_op,    // 如何把 logits 变成门控用的概率/分数
                 int   il,      // 当前 Transformer 层索引
         ggml_tensor * probs_in,    // 外部给定的路由 logits [n_expert, n_tokens]。
                                    // 非空时 不再 用 gate_inp * cur 算 logits（用于已预计算路由或特殊模型）。
         ggml_tensor * gate_up_exps,    // gate 与 up 融合 的大权重（[2*n_ff, ...] 一类），一次 mul_mat_id 再切成 gate/up 两段；
                                        // 非空则走融合分支，忽略单独的 gate_exps/up_exps 那条拆分路径。
    ...
    // select experts
    // 为每个 token 选出 top_k 个 expert id。
    ...
    // 这里的 weights 是当前 token，在 top_k 个 expert 上 做加权的权重。
    ...
    if (norm_w) {   // 对 top_k 个 expert 的权重按行归一，避免和为 0 时还会 clamp。
        weights = ggml_reshape_2d(ctx0, weights, n_expert_used, n_tokens);
    ...
    // 把 token 表示扩成“每个 token 对应多个专家分支”
    // 在mul_mat_id 算子中需要将 1 broadcast 到 top_k 个 expert 。
    cur = ggml_reshape_3d(ctx0, cur, n_embd, 1, n_tokens);

    // LLM_ARCH_LLAMA4 架构才会用到 weight_before_ffn，在 gate 前应用权重缩放。
    ...
    // 将 top_k 个expert计算得到结果 乘以 权重 weights，得到每个 token 在每个 expert 上的结果。
    if (!weight_before_ffn) {
...
ggml_tensor * llm_graph_context::build_inp_embd(ggml_tensor * tok_embd) const {
    const int64_t n_embd_inp = hparams.n_embd_inp();    // 输入嵌入的维度 + 辅助嵌入维度（如 deepstack）
    const int64_t n_embd     = hparams.n_embd;  // 输入嵌入的维度（模型维度）
    ...
    cb(inp->tokens, "inp_tokens", -1);  // -1 表示不在任何transformer层中
    ggml_set_input(inp->tokens);
    res->t_inp_tokens = inp->tokens;

    inp->embd = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_embd_inp, ubatch.n_tokens);
    cb(inp->embd, "inp_embd", -1);  // -1 表示不在任何transformer层中
    ...
    // token embeddings path (ubatch.token != nullptr)
    {
        ...
        // tok_embd: 词表权重矩阵
        // inp->tokens: 输入的token IDs
        // ggml_get_rows: 从词表权重矩阵中获取输入的token IDs对应的权重
    ...
    // ubatch.token 有值选路径 0（输入为token IDs），使用ggml_get_rows(rok_embd, tokens)  -- 从词表查找 
    // ubatch.token 有值选路径 1（输入为向量嵌入） -- 直接使用向量
...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        // Attention Logit Soft-capping（注意力分数软截断） 的特殊机制
        // 这个机制目前主要被 Google 的 Gemma 2（以及可能的 Gemma 3 等后续模型）所使用。 
        ...
        // Attention Sinks（注意力水槽/注意力汇） 特殊机制
        // 特定的模型结构才会触发这个机制，一般sinks为null
