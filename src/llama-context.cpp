...
llama_context::llama_context(
    ...
    if (!hparams.vocab_only) {
        ...
        for (auto & backend : backends) {
            ...
            if (reg) {
                // ggml_backend_set_n_threads 函数的作用是:
                //  在每次 graph_compute 前，把这些后端内部的并行线程数设成用户配置的 n_threads（或 batch 模式下的 n_threads_batch），
                //  主要影响 CPU/BLAS 等走多线程的路径，而不是 GPU 上传数据。
    ...
    if (!hparams.vocab_only) {
        ...
        // 在 host 上创建 KV cache tensor
        // 在 device 上创建 KV cache tensor 需要的 buffer，并为这些 tensor 的 data 选择一块区域
        // 预计算 attention rotation 用的 Walsh–Hadamard 正交矩阵，用来辅助量化 + RoPE
    ...
    if (!hparams.vocab_only) {
        ...
        bool pipeline_parallel =
            ...
            cparams.offload_kqv &&
            !model.has_tensor_overrides();  // 这个模型是不是有“按张量名单独改后端/缓冲区分配规则”的特殊覆盖配置？
...
// scheduler 指的是 GGML 的 ggml_backend_sched（llama_context 里的 sched）。
// 它负责在 多块设备 / 多种 backend 上跑 同一张计算图 时，把「谁在哪算、中间结果怎么搬、临时显存要多少」统一管理起来。
...
void llama_context::sched_reserve() {
    ...
    // 在还没真正搭完某次推理的图之前，先按「最坏大概有多大」把图结果缓冲和 scheduler 容量留够，
    // 避免后面 alloc_graph / reserve 时不够。
    const size_t max_nodes = this->graph_max_nodes(n_tokens);

    LLAMA_LOG_DEBUG("%s: max_nodes = %zu\n", __func__, max_nodes);

    // 主推理图
    // process_ubatch（约 1183–1252 行）里固定用 gf_res_prev：
    //  - 先 can_reuse(gparams)，拓扑不变则 只更新输入、不重搭整张图；
    //  - 否则 reset() + sched_reset + build_graph + alloc_graph。
    gf_res_prev.reset(new llm_graph_result(max_nodes));

    // 预留 / 辅助图
    // graph_reserve（sched_reserve、auto FA/FGDN 探测等）用 gf_res_reserve 去 build_graph，
    // 再 split_graph / sched_reserve，不碰 gf_res_prev。
    // 这样 探测最坏情况图、给 scheduler 定 buffer 时，不会把主推理里用于 **can_reuse 对比的那份 params/图状态弄乱。
    // 这是 额外一次小图，和当前 主 forward 图 分开，避免和 **gf_res_prev 抢同一份 gf/ctx。
    ...
    // 把 Gated Delta Net（一种线性注意力 / Delta Net 类块）做成融合算子 GGML_OP_GATED_DELTA_NET 的路径
    ...
    {
        // 按 decode（n_seqs 个 token，bs=1 量级） 再规划一遍；若某些中间张量/路径在 TG 上更大，池子会再扩容；
        // 同时 快照变成 TG 图（节点数、顺序往往和 PP 不同）
    ...
    // reserve again with pp graph to avoid ggml-alloc reallocations during inference
    {
        ...
        // 再用 同尺度的 PP 图 做一次 reserve，把 node_allocs / leaf_allocs 重新对齐到推理开始时实际会先跑的 PP 图，并视需要再扩池。
        // 这样在第一次真正跑 PP 时，ggml_gallocr_alloc_graph 更容易直接命中已有快照与足够大的池子，
        // 减少推理过程中因快照是 TG、图却是 PP 而触发的 realloc/失败路径
...
int llama_context::decode(const llama_batch & batch_inp) {
    GGML_ASSERT((!batch_inp.token && batch_inp.embd) || (batch_inp.token && !batch_inp.embd)); // NOLINT
    // 若 memory == nullptr，说明当前模型/上下文 不按「带 cache 的解码」来做，于是 降级调用 encode()：
    // create_memory 直接返回 nullptr（最常见于「不需要标准 KV cache」的架构）,
    // 这些模型做的是 双向 / encoder 式前向（拿 embedding 或 encoder 输出），不做自回归解码用的 KV 累加。
    if (!memory) {
    ...
    // sampling.samplers 非空，表示至少有一条序列通过 llama_set_sampler(ctx, seq_id, smpl) 绑定了采样链（实验特性，常见入口是 -bs / --backend-sampling）。
    // 这类采样会在 图计算 / 设备侧 参与，而不是完全在 CPU 上对整块 logits 自己做 argmax。
    const bool has_samplers = !sampling.samplers.empty();

    const uint32_t n_seq_max = cparams.kv_unified ? LLAMA_MAX_SEQ : cparams.n_seq_max;

    // TODO: avoid this workaround in the future
    // 针对的是 「已启用 Graph/设备侧采样（Backend sampling）」 时的 输入合法性校验，
    // 不是普通纯 CPU 采样的默认路径。
...
uint32_t llama_context::graph_max_nodes(uint32_t n_tokens) const {
    ...
    uint32_t res = std::max<uint32_t>(1024u, 8u*model.n_tensors()); // 经验公式, 8 是经验系数
...
ggml_cgraph * llama_context::graph_reserve(
    ...
    // 造一张「预留/测量用」的假 ubatch
    // 把总 token 数拆成 n_seq_tokens = n_tokens / n_seqs（每条约多少 token）× n_seqs（几条序列）。
    // 很多图构建路径（包括注释里提到的 pooling）都假设 ubatch.n_tokens == n_seq_tokens * n_seqs，
    // 且各条序列 token 数对齐（equal_seqs 一类布局）
    if (n_tokens % n_seqs != 0) {
        // 把 n_tokens 向上取整到 n_seqs 的倍数，保证「每序列同长」的假设成立
    ...
    // 喂给 build_graph 的一套合法、规整、且按你配置的 batch/ctx 上界选出来的形状；
    // 预留的是 执行该规模图所需的中间张量 compute 内存池，不是 KV、不是权重。
