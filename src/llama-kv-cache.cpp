...
llama_kv_cache::llama_kv_cache(
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            ggml_init_params params = {
                // 这里的 1 表示 每层里那一对 “根张量”本身： [n_embd_k_gqa, kv_size, n_stream]
                // 然后在这个 3D 根 tensor 上，为每个 stream 切出一个 2D 视图。 -- 各个 stream 是 view ！！！
    ...
    //  v_heads作用: 按 stream（或 unified 时唯一的 stream）维护的一根「写指针 / 下次找空槽的起点」，
    // 下标落在 v_cells[s] 这一维 cell 数组里。
    // 每个并行 KV stream 各自一根 head（unified 时 n_stream == 1，只有一根）
    v_heads.resize(n_stream);
    for (uint32_t s = 0; s < n_stream; ++s) {
        v_heads[s] = 0;
    }

    // v_cells 是 KV cache 的「槽位元数据」表：和 cache_k / cache_v 里沿 kv_size 那一维的每一格一一对应，
    // 用来回答「第 i 格现在存的是谁的、第几个 token、空不空、能不能被覆盖」，不存 K/V 向量本身（向量在大张量里）。
    ...
    if (n_stream > 1) {
        ...
        for (uint32_t s = 0; s < n_stream; ++s) {
            seq_to_stream[s] = s;   // 第几个 seq 映射到 第 s 个 stream
        }
    }
    ...
    for (uint32_t il = 0; il < hparams.n_layer; il++) {
        ...
        // 为当前层分配一个 ggml context
        // 同一 buft 的层复用同一个 context, 避免每层一个独立 context/buffer, 更适合多设备、多层混合 offload。
        ...
        // size_t(2u*(1 + n_stream)*n_layer_kv*ggml_tensor_overhead())   1 就是表示 这里 的 k 和 v
        ...
        // k_stream、v_stream 也是一种 tensor，记录了属于当前层所有的 K V tensor
        for (uint32_t s = 0; s < n_stream; ++s) {
    ...
    for (auto & [buft, ctx] : ctx_map) {
        // ctx 中存放了 所有层、所有 stream 自己二维的 k、v，以及 没有分 stream 时的三维 k、v
        ...
        if (model.hparams.no_alloc) {
            ...
            // 在 buft 下为 ctx 下所有的 tensor 分配一块 buffer
    ...
    // 预计算的是 attention rotation 用的 Walsh–Hadamard 正交矩阵，
    // 给 量化 K/V 在 RoPE/shift 路径上「先旋转 → RoPE → 再旋转回去」 用
    // 作用： 减小量化+RoPE 数值误差的辅助旋转
    attn_rot_k =
        !attn_rot_disable &&
        n_embd_head_k_all > 0 &&
...
// 1 如果 src 和 dst 在同一个 stream，只要改 metadata，不需要真拷贝 K/V buffer。
// 2 如果在不同 stream，则需要把整条 stream 的 K/V buffer 从 ssrc 拷到 sdst。
// 3 这个拷贝不会立刻做，而是先记录到 sc_info，等下一次 init_update() / update() 时统一执行。
...
// 查询「逻辑序列 seq_id 在 KV 里已经出现过的最大 llama_pos」
...
// 切分策略：
//   1 n_stream == 1：balloc.split_simple(n_ubatch)
//   2 n_stream > 1：balloc.split_equal(n_ubatch, true)
// 之所以 multi-stream 要 split_equal(..., true)，是因为：
//   1 每个 stream 实际对应一个 sequence。
//   2 需要保证 ubatch 中各 stream 对齐，便于后续 slot_info 和图输入构建。
// 若：  batch 不能被完全切掉, 或者 prepare(ubatches) 失败。
...
llama_memory_context_ptr llama_kv_cache::init_batch(
    ...
    do {
        ...
        // 为什么要这么做？
        //   1 后续 ubatch 的找槽位，依赖前面 ubatch 已经占掉了哪些槽。
        //   2 但真正提交前又不能破坏当前 cache。
        // 因此需要：
        //   1 先模拟。
        //   2 若全成功，再在运行时逐个 apply()。
...
//   2 对每个参与的 seq/stream：
//     1 从 v_heads[strm] 起开始扫。
//     2 优先找空 cell。
//     3 若 cell 非空但满足 SWA 覆盖条件，也可复用。
...
llama_kv_cache::slot_info llama_kv_cache::find_slot(const llama_ubatch & ubatch, bool cont) const {
    ...
    // 在一个 ubatch 中，可以有多个 stream 的token
    // 每个 stream 的 token 数量必须相同
    if (n_stream > 1) {
...
void llama_kv_cache::apply_ubatch(const slot_info & sinfo, const llama_ubatch & ubatch) {
    ...
    for (uint32_t s = 0; s < sinfo.n_stream(); ++s) {
        // sinfo.size(): 当前 stream 的 token 数量，
        // 因为在ubatch中，每个stream的token数量相同，所以 sinfo.size() 就是每个 stream 的 token 数量
        for (uint32_t ii = 0; ii < sinfo.size(); ++ii) {
            const uint32_t i = s*sinfo.size() + ii; // 当前 stream 的 token 在 ubatch 中的索引

            // 找到当前 stream 对应的 cells
            auto & cells = v_cells[sinfo.strm[s]];

            // 找到当前 stream 的 token 在 cells 中的索引
            const auto idx = sinfo.idxs[s][ii];

            if (!cells.is_empty(idx)) {
                // 如果当前 cell 不为空，则需要删除该 cell
                assert(cells.seq_count(idx) == 1);

                // 获取当前 cell 的序列 id 和位置
...
ggml_tensor * llama_kv_cache::get_k(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    const uint64_t n_embd_k_gqa = k->ne[0];     // 将所有 kv_heads 的 K cache 按照顺序排列起来
