...
bool llama_batch_allocr::init(
    ...
    for (int32_t i = 0; i < batch.n_tokens; ++i) {
        ...
        for (int32_t s = 0; s < batch.n_seq_id[i]; ++s) {
            ...
            // seq_pos 记录当前 seq 中的 tokens 在 batch 中的位置
            seq_pos[s1].insert(batch.pos[i]);

            // 若某一行 n_seq_id > 1，说明这一行上的同一个 (token, pos) 同时挂在多条序列上；
            // 把「多出来的那些序列」标记为与该行第一个序列 seq_id[i][0] 耦合。
            if (s > 0) {
    ...
    // 用 std::bitset<LLAMA_MAX_SEQ>（类型别名 seq_set_t） 把「每一行 i 挂了哪些 seq_id」压成一个固定位宽的集合，
    // 方便后面 按「序列集合模式」分组、切 micro-batch。

    // 为当前 llama_batch 预先算出：
    //  1 每 token 的 序列集合指纹（seq_set）；
    //  2 每种指纹对应 哪些行下标（ seq_set_map ，供切 ubatch 时分组）；
    //  3 本 batch 出现过哪些 seq_id 及其 紧凑编号（seq_id_unq / seq_idx）。
    // std::bitset<LLAMA_MAX_SEQ> 在这里就是「每行一个小的 seq 集合 + 可当哈希表键的指纹」，把后面 split / 按序列集合处理 batch 的逻辑从「反复扫 seq_id 数组」变成「按模式聚合」。
    {
        seq_set_t seq_set_unq;

        for (int32_t i = 0; i < batch.n_tokens; ++i) {
            seq_set_t cur;  // 当前 token 在从 0 到 LLAMA_MAX_SEQ 范围内 ，都有哪些 seq_id
            for (int32_t s = 0; s < batch.n_seq_id[i]; ++s) {
                const llama_seq_id seq_id = batch.seq_id[i][s];
        ...
        // seq_id_unq：本 batch 实际出现过的 seq_id，按 s 从小到大枚举的顺序（0,1,2,… 谁先 test 谁先 push）。
        // seq_idx[s]：只在「出现过的 s」上有定义意义：它是 s 在 seq_id_unq 里的下标，给后面按「唯一 seq 维度」做池化、索引之类用。
        for (uint32_t s = 0; s < n_seq_max; ++s) {
            if (seq_set_unq.test(s)) {
                seq_idx[s] = seq_id_unq.size();     // 当前是第几个「唯一 seq」
                seq_id_unq.push_back(s);            // 将当前seq_id加入到seq_id_unq中
            }
        }
    }
