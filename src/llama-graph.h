...
public:
    ...
    ggml_tensor * t_inp_tokens  = nullptr;  // 整型 token 输入张量
    ggml_tensor * t_inp_embd    = nullptr;  // 连续向量输入, shape: [n_embd_inp, n_tokens]
    ggml_tensor * t_logits      = nullptr;  // 语言模型头输出
    ggml_tensor * t_embd        = nullptr;  // 每个 token 的隐藏状态
    ggml_tensor * t_embd_pooled = nullptr;  // 池化后的句子/序列级向量（如 CLS、mean pooling、rank 等）

    std::map<llama_seq_id, ggml_tensor*> t_sampled_logits;  // 采样用 logits 子集
    std::map<llama_seq_id, ggml_tensor*> t_candidates;      // 候选 token id 列表
    std::map<llama_seq_id, ggml_tensor*> t_sampled;         // 采样得到的 token id（或同类离散结果）。
    std::map<llama_seq_id, ggml_tensor*> t_sampled_probs;   // 对应 概率（或分布片段）。

    std::vector<llm_graph_input_ptr> inputs;    // 把 KQ mask、pos、KV 相关 input、embd input 等封装成统一接口。

    // 本张推理图专用的 ggml_context，no_alloc = true，张量元数据从下面的 buf_compute_meta 里划。
    // 所有 ggml_new_tensor_* / 算子节点 都挂在这个 context 上，直到 reset() 重建
    ggml_context_ptr ctx_compute;

    // 作为 ctx_compute 的 mem_buffer
    // 不经过 malloc 堆扩张，用一块连续 buffer 存本图所需的 tensor 头 + graph 元数据，大小由 max_nodes 上限决定。
    // memory buffers used to evaluate the model
    std::vector<uint8_t> buf_compute_meta;

    // 当前这次 llm_graph_result 对应的 计算图（节点拓扑、执行顺序）
    ggml_cgraph * gf;

    int64_t max_nodes;  // 构图允许的 节点数上限

private:
    // 上一次成功构建图时的 llm_graph_params 快照
    // can_reuse 里与新的 params 比较（allow_reuse + 各 inputs 形状兼容），决定 能否复用同一张图。
