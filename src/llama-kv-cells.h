...
private:
    ...
    std::set<uint32_t> used;     // 已使用 cell 的下标集合（pos != -1 等语义），便于遍历/调试。

    std::vector<llama_pos> pos;     // 第 i 个 cell 上的 逻辑位置（空槽常为 -1）

    // stores extra info per cell
    std::vector<llama_kv_cell_ext> ext;     // 每个 cell 额外元数据
    ...
    //   }
    //
    // 对 pos 做 位移/除法 等批量改写时的暂存队列，
    // 最后可一次应用（RoPE 上下文扩展、窗口滑动等场景）。
    std::vector<llama_pos> shift;

    using seq_set_t = std::bitset<LLAMA_MAX_SEQ>;

    // the bitset seq[i] tells us which sequences are currently occupying the i-th cell
    std::vector<seq_set_t> seq; // 第 i 个 cell 当前被哪些 seq_id 占用；多数情况 1 个 bit。
    ...
    //
    // 对固定序列 s：位置 p 在 cache 里出现了多少次。
    // 注释说明了为什么不用 set：同一 (s,p) 可出现多次（cache 复用 rm+add、视觉模型重复位置等）。
    // 最小/最大位置用 begin() / rbegin() 的键，且要求 count > 0。
