...
static buft_list_t make_cpu_buft_list(const std::vector<llama_device> & devices, bool use_extra_bufts, bool no_host) {
    ...
    // 这些 extra buft 包括（视编译与平台而定）：权重 repack 专用 buffer、AMX 等 CPU 加速路径的 buffer 等，都挂在 CPU 设备上
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // 对于 offload 的 transformer block，多卡切分场景，用于 **PP** 按层切分模型
    //  CPU 不参与这张比例表，而是由 -ngl 决定有多少层留在 CPU。
    // 两张卡希望大约 3:7，可写 --tensor-split 3,7
    // 全写 0 或不设 则按各卡 空闲显存 自动分配比例。
    ...
    // 后面会填 第几个文件 idx → 哪块 ggml_backend_buffer_t，
    // 供后面 set_tensor_data 知道从哪个 mmap 段拷、挂到哪块 buffer
    ...
    for (auto & [buft, ctx_ptr] : ml.ctx_map) {
        ...
        if (ml.use_mmap && use_mmap_buffer && buffer_from_host_ptr_supported && is_default_buft) {
            ...
            if (ml.no_alloc) {
                // 此时不需要、也不应该让各后端去 alloc 一大块内存
                // 让 backend 调度/分配逻辑 看到张量已经「属于」某个 buffer，而不会再按「无 buffer」路径去自动给它们申请空间。
                // 占位 buffer 的 buft 仍然是对的，这样张量仍带有「计划放在 CPU / GPU 哪种 buffer 类型上」的语义
                buf = ggml_backend_buft_alloc_buffer(buft, /*size =*/ 0); // dummy buffer
                for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
                    t->buffer = buf; // set dummy buffer for weights so that the backend scheduler won't try to allocate them
                }
            } else {
                // 对当前这个 ctx + buft 在对应后端上分配权重 buffer 并给张量绑好地址
                buf = ggml_backend_alloc_ctx_tensors_from_buft(ctx, buft); // 为tensors在device上申请空间并初始化
            }
            if (buf == nullptr) {
                throw std::runtime_error(format("unable to allocate %s buffer", ggml_backend_buft_name(buft)));
            }
            ...
            bufs.emplace_back(buf); // 
            for (uint32_t idx = 0; idx < ml.files.size(); idx++) {
                buf_map.emplace(idx, buf);  // 每一个 GGUF 文件 idx → 哪块 ggml_backend_buffer_t，
            }
        }
    ...
    if (llama_supports_gpu_offload()) {
        ...
        const int n_offloaded_transformer  = std::max(0, std::min(n_layer - i_gpu_start, act_gpu_layers));
        const bool offload_output          = (n_layer >= i_gpu_start) && ((n_layer - i_gpu_start) < act_gpu_layers);
        const int n_offloaded              = n_offloaded_transformer + (offload_output ? 1 : 0);

        if (n_offloaded_transformer > 0) {
            LLAMA_LOG_INFO("%s: offloading %d repeating layers to GPU\n", __func__, n_offloaded_transformer);
        }
        if (offload_output) {
            LLAMA_LOG_INFO("%s: offloading output layer to GPU\n", __func__);
        }
        LLAMA_LOG_INFO("%s: offloaded %d/%d layers to GPU\n", __func__, n_offloaded, max_offloadable_layers);
    }
        ...
        default:
            {
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE) {
                        ...
                        // 1. 在 host上为每个stream、每层transformer block的整体 k、v（三维，不区分stream）以及区分stream的k、v（二维）分配空间
                        // 2. 在 buft 下为 ctx 下为 1 中所有的 tensor 分配一块buffer
                        // 3. 在K V量化存储的条件下，预计算Walsh–Hadamard 正交矩阵，用来辅助量化 + RoPE
