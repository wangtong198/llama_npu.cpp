...
bool llama_supports_gpu_offload(void) {
    return ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_GPU) != nullptr ||
           ...
           ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_AWNPU) != nullptr ||
           llama_supports_rpc();
}
        ...
        if (params.split_mode == LLAMA_SPLIT_MODE_TENSOR) {
            ...
            for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
                ...
                switch (ggml_backend_dev_type(dev)) {
                    ...
                    case GGML_BACKEND_DEVICE_TYPE_AWNPU:
                        // AWNPU follows the same default device collection path as discrete GPU:
                        // discover first, then initialize per-device backend only when selected.
                        gpus.push_back({false, dev});
                        break;
