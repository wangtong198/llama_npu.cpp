...
extern "C" {
    ...
    typedef struct llama_batch {
        // 本 batch 里 token 个数  
        // 下面所有「按行展开」的指针，逻辑长度都是 n_tokens（ seq_id 是「每行一个指针」，指向该行的 seq 列表）。
        int32_t n_tokens;

        // // token 与 embd（二选一）
        // token[i]：第 i 行的 离散 token id
        llama_token  *  token;  
        // 若走连续输入，指向 按行展开 的浮点向量；
        // 通常理解为 [n_tokens * n_embd] 或等价的 n_embd 维向量首尾相接（具体 n_embd 由模型上下文决定）
        float        *  embd;
        // pos[i]：第 i 行在 所属序列里的位置（llama_pos，一般是整型下标，
        // 多维 RoPE 时在内部可能扩展为每 token 多个 position，由实现补全）
        // 多序列 batch 时每一行通过自己的 seq_id 决定「这条 pos 是哪一个序列里的第几个位置」
        llama_pos    *  pos;
        // n_seq_id[i]：第 i 行 挂在了几个序列上
        // （一般为 1；大于 1 表示 同一次前向、同一 token 被多条序列共享，例如特定批并行 / 耦合场景）
        int32_t      *  n_seq_id;
        // seq_id[i]：指向长度至少为 n_seq_id[i] 的 llama_seq_id 数组，给出该行归属的序列 id。
        llama_seq_id ** seq_id;
